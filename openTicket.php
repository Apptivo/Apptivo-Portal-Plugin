 ...
 */
function open_ticket_form(){
	echo "Open ticket entered.. exiting ";exit;
...
$data=getAllCasesConfigData();
$statuses=$data->statuses;
//echo '<pre>';print_r($statuses);
foreach($statuses as $status){
	if($status->statusCode =="OPEN"){
		$open_status_id=$status->statusId;
	}
		 if($status->statusCode == "IN PROGRESS"){
		$inprogress_status_id=$status->statusId;
	}
}

//echo $open_status_id;
//echo $inprogress_status_id;
//echo '<pre>';print_r($status_id);
//echo '<pre>';print_r($status_name);
$customerName=$_SESSION['customerName'];
//echo $customerName;

$customerId=$_SESSION['customerId'];
//echo $customerId;
$myCases=get_all_open_tickets($open_status_id,$inprogress_status_id,$customerId);
//po1($myCase);
